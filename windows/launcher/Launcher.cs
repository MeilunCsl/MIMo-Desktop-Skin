...
public sealed class DreamLauncher : Form {
  ...
  public DreamLauncher(string root, string mode, string port, string preview) {
    ...
    RoundWindow();
    var icon=Path.Combine(root,"logo","chatgpt_white_transparent_windows.ico");
  ...
  async Task Initialize() {
    try {
      var env=await CoreWebView2Environment.CreateAsync(null,Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"CodexDreamSkin","launcher-webview"));
  ...
  async Task UpdateProgress() {
    ...
    if(done) {
      ...
      if(failed||cancelled) {
        string folder=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"CodexDreamSkin");
