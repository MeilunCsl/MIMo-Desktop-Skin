...
function Enter-DreamSkinOperationLock {
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    throw 'Another Codex Dream Skin start or restore operation is already running.'
  }
  return $mutex
}
...
function Get-DreamSkinRegisteredCodexInstalls {
  # Windows can return no package for the implicit current-user query from a
  # non-interactive PowerShell host even though the signed Store package is
  # installed and the desktop app is running. Resolve the package explicitly
  # for the current user before validating its manifest and identity.
  $currentUser = $env:USERNAME
  $packages = @(Get-AppxPackage -User $currentUser -Name 'OpenAI.Codex' -ErrorAction Stop |
...
function Start-DreamSkinCodexExecutable {
  param(
    [Parameter(Mandatory = $true)][object]$Codex,
    [AllowEmptyCollection()][string[]]$Arguments = @()
  )
  ...
  if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
    throw "The verified Codex executable was not found: $executable"
  }
  $process = Start-Process -FilePath $executable -ArgumentList $Arguments -PassThru
  if ($null -eq $process -or $process.Id -le 0) {
    throw 'Windows did not return a Codex process ID after direct package launch.'
  }
  return $process.Id
}
...
function Stop-DreamSkinRecordedInjector {
  ...
  if (-not $processPath -or -not $commandLine) {
    Write-Warning "The recorded injector PID $processId is running, but its identity cannot be inspected; treating the saved state as stale without touching that process."
    return $false
  }
  ...
  if (-not $identityMatches) {
    Write-Warning "The recorded injector PID $processId no longer matches the saved Dream Skin process; treating the saved state as stale without touching that process."
    return $false
  }

  Stop-Process -Id $processId -Force -ErrorAction Stop
  # Windows PowerShell 5.1 can return from Wait-Process before a Node process
  # with redirected log handles disappears from Get-Process. Poll the visible
  # PID directly so normal delayed teardown is not reported as a failure.
...
function Get-DreamSkinWatchInjectors {
  param(
  ...
  $themePath = [System.IO.Path]::GetFullPath($ThemeDirectory)
  $pausePath = [System.IO.Path]::GetFullPath($PauseFile)
  return @(Get-CimInstance Win32_Process -Filter "Name = 'node.exe'" -ErrorAction SilentlyContinue |
    Where-Object {
      $processPath = Get-DreamSkinProcessExecutablePath -ProcessInfo $_
      $commandLine = "$($_.CommandLine)"
      if (-not $processPath -or -not $commandLine -or
        [System.IO.Path]::GetFileName($processPath) -ine 'node.exe') {
        return $false
      }
      if ($commandLine -notmatch '(?i)\\windows\\scripts\\injector\.mjs(?:["\s]|$)' -or
        -not (Test-DreamSkinCommandLineToken -CommandLine $commandLine -Token '--watch') -or
        -not (Test-DreamSkinCommandLineToken -CommandLine $commandLine -Token $themePath) -or
        -not (Test-DreamSkinCommandLineToken -CommandLine $commandLine -Token $pausePath)) {
        return $false
      }
      $portPattern = '(?i)(?:^|\s)--port(?:=|\s+)' + [regex]::Escape("$Port") + '(?=$|\s)'
      if (-not [regex]::IsMatch($commandLine, $portPattern)) { return $false }
      if ($BrowserId) {
        $browserPattern = '(?:^|\s)(?i:--browser-id)(?:=|\s+)' + [regex]::Escape($BrowserId) + '(?=$|\s)'
...
function Stop-DreamSkinWatchInjectors {
  ...
  $matches = @(Get-DreamSkinWatchInjectors -Port $Port -ThemeDirectory $ThemeDirectory `
    -PauseFile $PauseFile -BrowserId $BrowserId)
  $stopped = @()
  foreach ($match in $matches) {
    ...
    if (Get-Process -Id $processId -ErrorAction SilentlyContinue) {
      throw "A Dream Skin watcher did not stop: PID $processId"
    }
    $stopped += $processId
  }
  return @($stopped)
