...
$RendererSource = Join-Path $SourceRoot 'windows\assets\renderer-inject.js'
$StyleSource = Join-Path $SourceRoot 'windows\assets\dream-skin.css'
...
try {
  ...
  try {
    ...
    if ($null -eq (Get-DreamSkinVerifiedCdpIdentity -Port $Port -Codex $codex)) {
      ...
      if ($ProfilePath) {
        ...
        $launchArguments += "--user-data-dir=$ProfilePath"
      }
      $null = Start-DreamSkinCodex -Codex $codex -Arguments $launchArguments
      $launchedWithCdp = $true
    }

    $deadline = (Get-Date).AddSeconds(45)
    $directFallbackAt = (Get-Date).AddSeconds(8)
    ...
    while ($null -eq $cdpIdentity) {
      ...
      if ($launchedWithCdp -and -not $directFallbackUsed -and
        $launchArguments.Count -gt 0 -and (Get-Date) -ge $directFallbackAt) {
        # Some Store activations bring the single-instance app to the front but
        # drop command-line switches. Retry once with the exact executable from
        # the already validated Store package, avoiding an endless flashing
        # open/close loop while preserving the same package identity checks.
        Write-Warning 'Codex opened without the requested CDP endpoint; retrying the verified package executable once.'
        Stop-DreamSkinCodex -Codex $codex -AllowForce
        if (-not (Test-DreamSkinPortAvailable -Port $Port)) {
          if ($PortExplicit) { throw "Port $Port is still occupied after the CDP launch retry." }
          $Port = Select-DreamSkinPort -PreferredPort $Port
          $launchArguments = @('--remote-debugging-address=127.0.0.1', "--remote-debugging-port=$Port")
          if ($ProfilePath) { $launchArguments += "--user-data-dir=$ProfilePath" }
          $deadline = (Get-Date).AddSeconds(45)
        }
        $null = Start-DreamSkinCodexExecutable -Codex $codex -Arguments $launchArguments
        $directFallbackUsed = $true
        $directFallbackAt = [DateTime]::MaxValue
      }
    ...
    if ($launchedWithCdp -and $null -ne $previousState -and $previousState.injectorPid) {
      $oldInjector = Get-Process -Id ([int]$previousState.injectorPid) -ErrorAction SilentlyContinue
      $oldPort = @(Get-NetTCPConnection -LocalPort $Port -ErrorAction SilentlyContinue)
      if ($null -eq $oldInjector -and $oldPort.Count -eq 0) {
        try {
          $staleStatePath = Archive-DreamSkinStateFile -Path $StatePath
          Write-Warning "Archived stale Dream Skin state at $staleStatePath"
        } catch {
          Write-Warning 'The stale Dream Skin state could not be archived; it will be reconciled on the next launch.'
        }
      }
    }
    throw $launchError
  ...
  $temporaryPauseSet = $false
  try {
    $existingWatchers = @(Get-DreamSkinWatchInjectors -Port $Port -ThemeDirectory $themePaths.Active `
      -PauseFile $themePaths.PauseFile)
    if ($existingWatchers.Count -gt 0) {
      # A hard Node termination leaves its Page.addScriptToEvaluateOnNewDocument
      # registration inside the current renderer. Pause first so every watcher
      # removes both the live payload and its early script before it is stopped.
      Set-DreamSkinPaused -Paused $true -StateRoot $StateRoot | Out-Null
      $temporaryPauseSet = $true
    ...
    # Reconcile managed watchers by their stable script/port/theme contract;
    # the browser id in a stale state file is not a safe ownership signal.
    # An unrelated process is never stopped: Get-DreamSkinWatchInjectors only
    # matches the Dream Skin injector command line and managed paths.
    $orphanedInjectors = @(Stop-DreamSkinWatchInjectors -Port $Port -ThemeDirectory $themePaths.Active `
      -PauseFile $themePaths.PauseFile)
    if ($orphanedInjectors.Count -gt 0) {
      Write-Warning "Stopped unrecorded Dream Skin watcher PID(s): $($orphanedInjectors -join ', ')"
    }
  } catch {
    if ($temporaryPauseSet -and -not $pauseWasSet) {
      try { Set-DreamSkinPaused -Paused $false -StateRoot $StateRoot | Out-Null } catch {
        Write-Warning 'Watcher reconciliation failed and the temporary pause marker could not be cleared.'
      }
    }
    if ($launchedWithCdp) {
      try {
  ...
  if ($ForegroundInjector) {
    try {
      ...
      & $node.Path $Injector --watch --port $Port --browser-id $cdpIdentity.BrowserId `
        --theme-dir $themePaths.Active --pause-file $themePaths.PauseFile --state-file $StatePath
  ...
  try {
    $injectorArgs = @((ConvertTo-DreamSkinProcessArgument -Value $Injector), '--watch', '--port', "$Port",
      ...
      (ConvertTo-DreamSkinProcessArgument -Value $themePaths.PauseFile), '--state-file',
      (ConvertTo-DreamSkinProcessArgument -Value $StatePath))
    ...
    $state = [pscustomobject]@{
      ...
      injectorHash = (Get-FileHash -LiteralPath $Injector -Algorithm SHA256).Hash.ToLowerInvariant()
      rendererHash = (Get-FileHash -LiteralPath $RendererSource -Algorithm SHA256).Hash.ToLowerInvariant()
      styleHash = (Get-FileHash -LiteralPath $StyleSource -Algorithm SHA256).Hash.ToLowerInvariant()
      nodePath = $node.Path
    ...
    while ($true) {
      ...
      if (-not (Test-DreamSkinCodexPortOwner -Port $Port -Codex $codex)) {
        throw "Codex CDP endpoint disappeared during Dream Skin verification on port $Port. See $VerifyPath"
      }
